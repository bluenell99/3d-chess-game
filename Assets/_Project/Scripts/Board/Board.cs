...
public class Board
{
    ...
    public List<Piece> PiecesInPlay { get; set; }
    ...
    public void AddPiece(Piece piece)
    {
        ...
        {
            Debug.Log("Piece already in play");
            return;
        }
    ...
    /// <summary>
    /// Returns the piece from a given square
    /// </summary>
    /// <param name="coordinate"></param>
    /// <returns></returns>
    public Piece GetPieceFromSquare(Vector2Int coordinate)
    {
        if (!IsSquareOccupied(coordinate)) return null;
    ...
    public HashSet<Move> EvaluateMoveLegality(HashSet<Move> possibleMoves, Piece piece)
    {
        ...
        // if the king isn't in check, we can return the possible moves
        if (!king.IsInCheck)
            return possibleMoves;

        // try and get the only one checking piece
        Piece checkingPiece = GetSingleCheckingPiece(piece);

        // if there is more than one checking piece, no move from this piece could block the check, so return an empty HashSet<Move>
        if (checkingPiece == null)
            return new HashSet<Move>();
        
        // get the legal moves from the checking piece
        HashSet<Move> checkingPieceMoves = checkingPiece.GetLegalMoves().Where(m => m.IsDeliveringCheck).ToHashSet();
        
        // return moves from this piece that will successfully capture or block the checking piece
    ...
    private Piece GetSingleCheckingPiece(Piece piece)
    {
        ...
        // Return null if there are multiple checking pieces or none (we handle this later in the main method)
        return null;
    }
